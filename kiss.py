...
class Kiss(commands.Cog):
    ...
    async def kiss(self, ctx, target: User):
        ...
        if ctx.message.author != target:
            await ctx.send(embed=Embed(description=f"(✿˘ω˘)˘ε˘˶ ) **{ctx.author.name}** kisses **{target.name}**",
                                       color=0x2F3136)
    ...
    async def kiss_error(self, ctx, error):
        ...
        if isinstance(error, MissingRequiredArgument):
            await ctx.send("Please specify someone to kiss with, it would be so sad otherwise ! 😢", delete_after=5.0)
